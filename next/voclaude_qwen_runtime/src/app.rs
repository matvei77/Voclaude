...
impl App {
    ...
    fn run_event_loop(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Backend preflight check: verify python & script paths resolve.
        // The actual model loading happens lazily in the inference worker via the
        // persistent server, so we don't start the server here.
        {
...
for qwen_model_path and CUDA availability.",
            ...
            // Server stays running - pass ownership to the inference worker.
            // Actually, preflight engine is separate from inference worker's engine,
            // so unload to avoid holding a second server process.
